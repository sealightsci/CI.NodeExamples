...
function requestHandler(request, response) {
  ...
  let requestedResource = path.join(
    ...
    resolveSourcesFolder(), // step out of this directory into the relevant directory
...
function resolveSourcesFolder(){
  const defaultFolder = "../dist";
  if(process.argv[2] && process.argv[2] === "--folder"){
    return process.argv[3] || defaultFolder;
  }
  return path.join(defaultFolder, "angular-calculator");
}
